...
CI (`.github/workflows/ci.yml`) runs `pnpm install`, `pnpm lint`, `pnpm build`, `pnpm test` on every push/PR. **Note:** `package.json` currently defines no `lint` or `test` script, and there is no ESLint/Prettier/Vitest config in the repo — if you're asked to add linting or tests, you'll need to wire up the tooling and scripts first, since CI expects `pnpm lint` and `pnpm test` to exist.

There is a Husky setup under `.husky/` but no custom hook scripts have been added yet (only the default husky.sh scaffolding).

## Architecture

**Stack:** Astro 7 (static output, no SSR adapter currently enabled) + React islands (`@astrojs/react`) + Tailwind CSS v4 (via `@tailwindcss/vite`, imported in `src/styles/global.css` rather than a `tailwind.config`) + TypeScript (strict Astro preset).
...
- **Post retrieval pattern:** every page that lists posts (`src/pages/index.astro`, `src/pages/blog.astro`, `src/pages/tags/[tag].astro`) independently calls `getCollection("posts", ({ data }) => !data.draft)` and sorts by `pubDate` descending using `.getTime()`. When adding a new post-listing page, follow this same filter+sort pattern for consistency (there's no shared helper yet — a good target if asked to reduce duplication).
- **Routing:**
  - `src/pages/posts/[...slug].astro` — individual post pages, driven by `getStaticPaths()` over the `posts` collection; renders content directly inline (via `render(entry)`) and computes a naive reading time (word count / 200 wpm). Note: `src/components/BlogPost.astro` is a small, currently-unused stub component — the real post template lives in this page file, not there.
...
- **Islands:** `src/components/Header/Header.tsx` is the one React island (`client:load` from `Layout.astro`). It owns the mobile nav toggle and the dark/light theme toggle, which persists to `localStorage` and toggles a `.dark` class on `<html>`. All other components are `.astro` (server-rendered, no hydration).
...
- `src/components/Search.astro` and `src/components/TableOfContents.astro` exist but are currently unused/placeholder — not imported by any page.
